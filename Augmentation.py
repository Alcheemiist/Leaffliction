...
def augment_and_save(image_path):
    ...
    augmentations = {
        ...
        'Skew': transforms.RandomAffine(degrees=0, shear=20),  # Skew implemented via RandomAffine
        'Shear': transforms.RandomAffine(degrees=0, shear=20),
        'Crop': transforms.RandomCrop((200, 200)),  # Assuming the crop size to be 200x200
        'Distortion': transforms.GaussianBlur(5)  # Using GaussianBlur as a form of distortion
    }
